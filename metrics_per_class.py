...
def get_tweet_preds(model, model_type, split, dataloader):
    if not os.path.exists(f"tweet_predictions_{split}_{model_type}.pkl"):
        model.eval()
        preds = []
        for sample in tqdm(dataloader):
            with torch.no_grad():
                ...
                preds.append(pred)
        with open(f"tweet_predictions_{split}_{model_type}.pkl", "wb") as f:
            pickle.dump(preds, f)
    with open(f"tweet_predictions_{split}_{model_type}.pkl", "rb") as f:
        tweet_preds = pickle.load(f)
    return tweet_preds

def get_scopus_preds(model, model_type, split, dataloader, trainer):
    if not os.path.exists(f"scopus_predictions_{split}_{model_type}.pkl"):
        ...
        for sample in tqdm(dataloader):
            with torch.no_grad():
                ...
                preds.append(pred)
        with open(f"scopus_predictions_{split}_{model_type}.pkl", "wb") as f:
            pickle.dump(preds, f)
    with open(f"scopus_predictions_{split}_{model_type}.pkl", "rb") as f:
...
if __name__ == '__main__':
    ...
    if eval_set == "scopus":
        ...
        threshold = get_threshold(train_preds, train_labels, eval_trainer)
        ...
        threshold = get_threshold(validation_preds, validation_labels, eval_trainer)
