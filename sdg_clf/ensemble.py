...
def test_ensemble(paths_models: list[str], model_types: list[str], dataset: str = "scopus", strategy: str = "mean", path_tokenizers: str = "tokenizers", path_pretrained_models: str = "../pretrained_models"):
    predictions = []
    if dataset == "scopus":
        tweet = False
    elif dataset == "tweets":
    ...
    for i, path_model, model_type in enumerate(zip(paths_models, model_types)):
        ...
        model_outputs_all = torch.stack(model_outputs_all, dim=0)
