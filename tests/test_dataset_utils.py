...
def test_process_twitter_df():
    # os.chdir("/home/victortolsagerolesen/programming/SDG_CLF")
...
def test_get_processed_df():
    ...
    tokenizer = dataset_utils.get_tokenizer("roberta-base")
    for i in range(len(df_twitter)):
        text = df_twitter.loc[i, "text"]
        try:
...
def test_get_processed_ds():
    ds = dataset_utils.get_processed_ds("twitter", "roberta-base", "test")
    assert set(ds.format["columns"]) == {"input_ids", "attention_mask", "label"}


def test_get_dataloader():
    dl = dataset_utils.get_dataloader("twitter", "roberta-base", "test")
    batch = next(iter(dl))
    assert batch["input_ids"].shape == batch["attention_mask"].shape == (20, 260)
    assert batch["label"].shape == (20, 17)
